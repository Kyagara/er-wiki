<script lang="ts">
	...
	import { getSorcery } from '$lib/sorceries.js';

	const { params } = $props();
	const sorcery = getSorcery(params.id);
</script>

<ItemPageHead item={sorcery} category="sorceries" type={'sorcery'} />

<div class="mx-auto max-w-6xl px-4 py-2">
	<a class="italic underline opacity-90 hover:text-white" href="/sorceries">
		{'<<'} Go back to sorceries</a
	>
	...
	<div class="grid grid-cols-1 items-start gap-8 pt-4 lg:grid-cols-3">
		<div class="flex flex-col gap-2 lg:col-span-2">
			...
			<LootLocations item={sorcery} />
		</div>

		<div class="lg:col-span-1">
			<Preview iconID={sorcery.iconID} />
		</div>
	</div>
</div>
