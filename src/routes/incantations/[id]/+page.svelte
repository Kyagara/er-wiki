<script lang="ts">
	...
	import { getIncantation } from '$lib/incantations.js';

	const { params } = $props();
	const incantation = getIncantation(params.id);
</script>

<ItemPageHead item={incantation} category="incantations" type={'incantation'} />

<div class="mx-auto max-w-6xl px-4 py-2">
	<a class="italic underline opacity-90 hover:text-white" href="/incantations">
		{'<<'} Go back to incantations</a
	>

	<div class="grid grid-cols-1 items-start gap-8 pt-4 lg:grid-cols-3">
		<div class="flex flex-col gap-2 lg:col-span-2">
			<Lore item={incantation} />
			...
			<LootLocations item={incantation} />
		</div>

		<div class="lg:col-span-1">
			<Preview iconID={incantation.iconID} />
		</div>
	</div>
</div>
